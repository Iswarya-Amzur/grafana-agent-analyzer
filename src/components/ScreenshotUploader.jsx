...
const ScreenshotUploader = () => {
  ...
  const fetchEmailConfig = async () => {
    try {
      const response = await fetch('http://localhost:8000/email_config/');
      if (response.ok) {
  ...
  const testEmail = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8000/test_email/', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        setUploadStatus('Test email sent successfully!');
      } else {
        setUploadStatus('Test email failed: ' + result.message);
      }
    } catch (error) {
      setUploadStatus('Test email error: ' + error.message);
    } finally {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Display email notification status if enabled
        if (sendEmail && result.email_notification) {
          const emailStatus = result.email_notification.success 
            ? '📧 Email notification sent!' 
            : '⚠️ Email notification failed: ' + result.email_notification.message;
          setUploadStatus(prev => prev + ' ' + emailStatus);
        }
