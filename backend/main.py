        ...
        if enable_llm_analysis and all_widgets:
            try:
                ...
                if analysis_result["success"]:
                    ...
                    if send_email:
                        try:
                            # Convert widgets to proper format for email
                            email_widgets = []
                            for widget in all_widgets:
                                if hasattr(widget, 'widget_name'):
                                    ...
                                    widget_obj = type('Widget', (), widget)()
                                    email_widgets.append(widget_obj)
                            
                            email_success = email_service.send_analysis_report(
                                analysis_result=analysis_result,
                                widgets_data=email_widgets,
                                excel_file=excel_file,
                            ...
                            response["email_notification"] = {
                                ...
                                "message": "Email sent successfully" if email_success else "Email sending failed"
                            }
                            ...
                            if critical_issues and len(critical_issues) > 0:
                                alert_success = email_service.send_critical_alert(
                                    critical_issues=critical_issues,
                                    immediate_actions=structured_data.get("recommendations", {}).get("immediate", [])
                                )
                                response["critical_alert"] = {
                                    "success": alert_success,
                                    "message": "Critical alert sent" if alert_success else "Critical alert failed"
                                }
...
async def send_email_notification(
    ...
    """
    Send email notification with analysis reports
    """
    try:
        if not recipients:
        ...
        email_recipients = [EmailRecipient(email=email.strip(), type="to") for email in recipients]
        
        # Check if files exist
        report_path = REPORTS_DIR / report_file if report_file else None
        excel_path = REPORTS_DIR / excel_file if excel_file else None
        ...
        # Mock analysis result for email (in real scenario, this would be stored/retrieved)
        mock_analysis = {
            "success": True,
            "analysis": {
                "structured_data": {
                    "performance_score": 7.5,
                    "issues": ["Sample critical issue"] if critical_only else [],
                    "recommendations": {
                        "immediate": ["Sample immediate action"],
                        "short_term": ["Sample short-term action"]
                    }
                },
                "model_used": "gpt-4o",
                "tokens_used": 1500
        ...
        if critical_only:
            ...
            success = email_service.send_critical_alert(
                critical_issues=["Critical system alert from manual email"],
                immediate_actions=["Review system immediately"],
            ...
            success = email_service.send_analysis_report(
                ...
                markdown_file=str(report_path) if report_path else None,
                recipients=email_recipients
            )
...
async def get_email_config():
    ...
    try:
        config_status = {
            "smtp_configured": bool(os.getenv("EMAIL_SMTP_USERNAME") and os.getenv("EMAIL_SMTP_PASSWORD")),
            "sendgrid_configured": bool(os.getenv("SENDGRID_API_KEY")),
            "from_email": os.getenv("EMAIL_FROM", "noreply@yourcompany.com"),
            "delivery_method": os.getenv("EMAIL_DELIVERY_METHOD", "smtp"),
            "recipients_file_exists": os.path.exists("recipients.json"),
            "config_file_exists": os.path.exists("email_config.json")
        }
        
        # Load recipients count
        if config_status["recipients_file_exists"]:
            try:
                with open("recipients.json", 'r') as f:
                    recipients = json.load(f)
                    config_status["recipients_count"] = len(recipients)
            except:
                config_status["recipients_count"] = 0
        else:
            config_status["recipients_count"] = 0
        ...
        raise HTTPException(status_code=500, detail=f"Config check error: {str(e)}")
...
async def test_email():
    ...
    try:
        ...
        success = email_service.send_critical_alert(
            critical_issues=["This is a test email from Grafana Analysis System"],
            immediate_actions=["No action required - this is a configuration test"],
            recipients=None  # Use default recipients
        )
